...
class NftCreator:
    ...
    #Returns the json template to use on the nfts
    def GetJsonTemplate(self):
        with open(os.path.dirname(__file__) + '/../input/template.json') as template:
            return json.load(template)

    #Creates the attributes list for the nfts and select the layer order that the nfts will use to create each nft
    def GetAttributesList(self):
        print('Obtainig attributes list ourand order of layers...', end = ' ', flush = True)
